def rss_cost(_hypothesis, reg_term):
    def cost_function(params, X, y):
        training_set_size = len(y)
        predictions = _hypothesis(X, params)
...
def rss_derivative(_hypothesis, reg_term):
...
def rss(_hypothesis, reg_term=0):
    return (rss_cost(_hypothesis, reg_term),
            rss_derivative(_hypothesis, reg_term))
